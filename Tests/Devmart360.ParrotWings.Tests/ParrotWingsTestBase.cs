...
namespace Devmart360.ParrotWings.Tests
{
    public abstract class ParrotWingsTestBase : AbpIntegratedTestBase<ParrotWingsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ParrotWingsTestBase()
        {
        ...
        protected void UsingDbContext(Action<ParrotWingsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ParrotWingsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ParrotWingsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ParrotWingsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ParrotWingsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ParrotWingsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ParrotWingsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ParrotWingsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ParrotWingsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ParrotWingsDbContext, Task<T>> func)
        {
            T result;
